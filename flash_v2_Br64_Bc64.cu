...
void flash_attention_2_forward_kernel(
    ...
    int row = tx / (Bc/4); // row from 0 to 63
    int col = tx % (Bc/4); // col from 0 to 15
    ...
    for (int i = 0; i < Tr; ++i) {
        ...
        for (int j = 0; j < Tc; ++j) {
            ...
            // for (int x = 0; x < 4; x++) {
            //     Kj[tx + 32 * 32 * x] = K[qkv_offset + (kv_tile_size * j) + tx + 32 * 32 * x];
            //     Vj[tx + 32 * 32 * x] = V[qkv_offset + (kv_tile_size * j) + tx + 32 * 32 * x];
            // }
            FLOAT4(Kj[row * d + col * 4]) = FLOAT4(K[qkv_offset + (kv_tile_size * j) + row * d + col * 4]);
