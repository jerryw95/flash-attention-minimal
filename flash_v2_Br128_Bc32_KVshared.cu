...
void flash_attention_2_forward_kernel(
    ...
    for (int i = 0; i < Tr; ++i) {
        ...
        // Causal mask: j <= i
        for (int j = 0; j < Tc; ++j) {
            int new_row = tx / 32; // row from 0 to 31
            int new_col = tx % 32; // col from 0 to 31
            __syncthreads();
            // Load Kj Vj from HBM to SRAM
            FLOAT2(KVj[new_row * d + new_col * 2]) = FLOAT2(K[qkv_offset + (kv_tile_size * j) + new_row * d + new_col * 2]);
            ...
            FLOAT2(KVj[new_row * d + new_col * 2]) = FLOAT2(V[qkv_offset + (kv_tile_size * j) + new_row * d + new_col * 2]);
            __syncthreads();
...
torch::Tensor forward(torch::Tensor Q, torch::Tensor K, torch::Tensor V) {
    ...
    const int Bc = 32; const int Br = 128;
