...
void flash_attention_2_forward_kernel(
    ...
    for (int i = 0; i < Tr; ++i) {
        ...
        for (int j = 0; j < Tc; ++j) {
            ...
            FLOAT2(Kj[new_row * d + new_col * 2]) = FLOAT2(K[qkv_offset + (kv_tile_size * j) + new_row * d + new_col * 2]);
            FLOAT2(Vj[new_row * d + new_col * 2]) = FLOAT2(V[qkv_offset + (kv_tile_size * j) + new_row * d + new_col * 2]);
