    ...
    for (int i = 0; i < Tr; ++i) {
        ...
        for (int j = 0; j < Tc; ++j) {
            ...
            for (int y = 0; y < 2; y++) {
                ...
                int col = col_base + y * Br;
                for (int x = 0; x < Bc; x++) {
                    pv += S[(Bc * row) + x] * Vj[(x * d) + col];
                }

                O[qkv_offset + (q_tile_size * i) + (row * d) + col] = \
                    row_m_exp * O[qkv_offset + (q_tile_size * i) + (row * d) + col] + pv;
            }
        ...
        for (int y = 0; y < 2; y++) {
            ...
            //    break;
            int col = col_base + y * Br;
            O[qkv_offset + (q_tile_size * i) + (row * d) + col] /= row_l_prev;
        }
