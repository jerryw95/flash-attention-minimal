...
void flash_attention_2_forward_kernel(
    ...
    for (int i = 0; i < Tr; ++i) {
        ...
        for (int j = 0; j < Tc; ++j) {
            ...
            for (int x = 0; x < d; x++) {
                Kj[(2*tx * d) + x] = K[qkv_offset + (kv_tile_size * j) + (2*tx * d) + x];
                Kj[((2*tx + 1) * d) + x] = K[qkv_offset + (kv_tile_size * j) + ((2*tx + 1) * d) + x];
                Vj[(2 * tx * d) + x] = V[qkv_offset + (kv_tile_size * j) + (2 * tx * d) + x];
                Vj[((2 * tx + 1) * d) + x] = V[qkv_offset + (kv_tile_size * j) + ((2 * tx + 1) * d) + x];
            }
            __syncthreads();
